...
import { BottomSheet, BottomSheetContent, BottomSheetHeader, BottomSheetTitle, BottomSheetDescription, BottomSheetBody } from "@/components/ui/bottom-sheet";
    ...
    <BottomSheetContent>
      ...
      <BottomSheetBody>
        <form className="space-y-4" onSubmit={e => { e.preventDefault(); save(true); }}>
          ...
          ] as const).map(([label, value, setValue]) => <label key={label} className="block text-sm font-medium">{label}<textarea className="mt-1 w-full rounded-lg border bg-background p-3 font-normal" rows={2} maxLength={1500} value={value} onChange={e => setValue(e.target.value)} /></label>)}
