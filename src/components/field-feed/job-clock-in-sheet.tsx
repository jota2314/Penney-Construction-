...
export function JobClockInSheet({
  ...
  // Keep the page behind the picker fixed and provide a reliable keyboard exit.
  // This is especially important on iOS, where a long list can otherwise scroll
  // the document instead of the sheet and leave the close button off-screen.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    listActivityMentions(job.id, { includeGroups: true })
      .then((rows) => {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center overflow-hidden overscroll-none sm:items-center"
      style={{ background: "rgba(0,0,0,0.6)" }}
      onClick={onClose}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="h-[92dvh] min-h-0 w-full overflow-hidden rounded-t-2xl sm:max-w-md sm:rounded-2xl flex flex-col"
        style={{
          background: v("card"),
          border: `1px solid ${v("line")}`,
          color: v("ink"),
          marginBottom: kbHeight,
          height: sheetMaxH ?? undefined,
        }}
      >
        {/* Header */}
        <div className="flex shrink-0 items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${v("line")}` }}>
          <div className="min-w-0">
          ...
          <button
            ...
            className="ml-3 flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full opacity-70 hover:opacity-100"
            style={{ background: v("bg-2"), color: v("ink") }}
          >
        ...
        {!job && (
          <>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain touch-pan-y px-3 pb-[max(1rem,env(safe-area-inset-bottom))] flex flex-col gap-1">
              {loadingJobs ? (
