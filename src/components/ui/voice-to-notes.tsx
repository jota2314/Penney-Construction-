 ...
 *
 * Caller decides what to do with the result via onResult.
 */
...
export function VoiceToNotes({
  ...
  useEffect(() => {
    if (!isListening && lastTranscript === null && transcript.trim()) {
      ...
      fetch("/api/structure-notes", {
        ...
        .then((r) => r.json())
        .then((data) => {
          onResult(typeof data.cleaned === "string" ? data.cleaned : final);
        })
        .catch(() => {
          onResult(final);
        })
  ...
  }, [isListening, transcript, lastTranscript, context, onResult]);
  ...
  return (
    ...
    >
      ...
      <span>
        {busy ? "Structuring…" : active ? "Stop" : (label ?? "Voice note")}
      </span>
    </button>
  );
}
