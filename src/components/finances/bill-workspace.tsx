...
import { LineItemPicker, type PickerLine } from "@/app/(app)/spent/[id]/line-item-picker";
import { ProjectPicker, type PickerProject } from "@/app/(app)/spent/[id]/project-picker";
...
export async function BillWorkspace({ id, embedded = false, month = "", paymentBlocked = false }: { id: string; embedded?: boolean; month?: string; paymentBlocked?: boolean }) {
  ...
  if (!inv) return embedded ? <p>This bill is unavailable or you no longer have access.</p> : notFound();
  ...
  return (
    <>
      {!embedded && <Header title={inv.vendor_name || "Transaction"} backHref="/spent" />}
            ...
            <div className="divide-y">
              {splitPieces.map((p) => {
                ...
                return isThis ? (
                  ...
                  <Link key={p.id} href={embedded ? `/finances/daily-log?month=${month}&panel=expense&id=${p.id}` : `/spent/${p.id}`} className={`${rowCls} hover:bg-muted/40 transition-colors`}>
                    {body}
                  </Link>
                );
              })}
