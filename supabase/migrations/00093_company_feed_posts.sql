-- Company feed: company-wide posts with optional project link, entity tags,
-- @mentions, and photos. A post must have text or at least one photo.
-- Anyone signed in can read; authors can only write/edit/delete their own.
...
CREATE TABLE IF NOT EXISTS public.company_feed_posts (
  ...
  CONSTRAINT company_feed_posts_has_content
    CHECK (
      ...
      OR cardinality(photo_storage_paths) > 0
    ),
