-- Preserve review holds when an office invoice becomes budget allocations.
-- Otherwise the children default to 'ok' and the flagged parent is deleted.
  ...
  loop
    ...
    with ins as (
      ...
      values (
        ...
        v_original.review_status, v_original.review_reason
      )
      returning *
    )
    select * from ins;
