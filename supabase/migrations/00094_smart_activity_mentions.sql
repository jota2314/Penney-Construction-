-- Durable @mentions for daily logs and an in-app notification inbox.
-- Push and email are delivery channels; this table is the source of truth
-- when a device has notifications disabled or an email cannot be sent.
...
-- This table exists in the live database but was missing from repository
-- history. Keep the migration idempotent so fresh environments can use push.
...
CREATE TABLE IF NOT EXISTS public.app_notifications (
  ...
  source_type text NOT NULL CHECK (source_type IN ('company_post', 'daily_log', 'project_update')),
